...
if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
	for (let parent of catalogParents){
		...
		parent.firstElementChild.addEventListener("click", slideToggle.bind(null, dropMenu));
	};
...
for (let checkbox of searchCheckboxes){
	checkbox.onchange = ()=>{
		...
		let checkboxActive = document.querySelectorAll(".categories__checkbox.active");
		if (checkboxActive.length > 0){
			searchTitle.classList.add("active");
			searchTitle.lastElementChild.innerHTML = searchTitle.lastElementChild.dataset.text + ' ' + checkboxActive.length;
		}
		else {
...
if (mainSliderElem){
	...
	let mainSliderDots = mainSliderElem.querySelectorAll(".swiper-pagination-bullet");
...
for (let i = toggleFilter.length - 1; i >= 0; i--) {
	let toggleCategory = toggleFilter[i].querySelector(".filter-toggle__category");
